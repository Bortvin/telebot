...
'''
@dp.message_handler()
async def create_user(message: [types.Message, types.BotCommand]):
...
async def key_add(message: types.Message):
    ...
    if len(words) <= 0:
        ...
        await message.answer(f'Слова "{words}" успешно добавлены')
...
async def users_add(message: types.Message):
    ...
    if len(words) <= 0:
        ...
        await message.answer(f'usernames"{words}" успешно добавлены')
